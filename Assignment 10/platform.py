...
NOTE: This file is named 'platform.py' per assignment requirements.
Python's stdlib also has a 'platform' module. When running from inside
this directory, our file shadows it. The workaround is to always run
this script as:
    python3 -m ai_security_platform.platform
OR simply invoke it directly:
    python3 platform.py   (Python's -m flag does NOT add CWD to path)

Running as:
    python3 platform.py --mode full          # run all modules end-to-end
    python3 platform.py --mode ingest        # ingestion only
